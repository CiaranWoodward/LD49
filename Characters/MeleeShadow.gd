...
func _handle_attacking():
	if (is_instance_valid(target_chunk) && is_instance_valid(target_chunk.occupant) && Global.is_adjacent_mc(map_chunk, target_chunk)):
		stateMachine.travel("FrontAttack")
		if target == Global.EnemyTarget.Crystal:
			Global.damage_crystal(10)
		else:
			target_chunk.occupant.damage((randi() % (maxdamage - mindamage + 1)) + mindamage)
	Global.next_enemy()
